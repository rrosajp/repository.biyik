...
def system_start_time():
  ...
  try:
    ...
    exc = IOError(f'unable to parse the /proc/stat btime entry: {btime_line}')
...
def cwd(pid):
  ...
  if pid == 0:
    ...
    try:
      ...
      exc = IOError(f'unable to read {proc_cwd_link}')
      _log_failure(parameter, exc)
      raise exc
...
def uid(pid):
  ...
  try:
    ...
    exc = IOError(f'unable to parse the {status_path} Uid entry: {uid_line}')
    _log_failure(parameter, exc)
    raise exc
...
def stats(pid, *stat_types):
  ...
  if len(stat_comp) < 44 and _is_float(stat_comp[13], stat_comp[14], stat_comp[21]):
    exc = IOError(f'stat file had an unexpected format: {stat_path}')
    _log_failure(parameter, exc)
    raise exc
...
def file_descriptors_used(pid):
  ...
  try:
    ...
    raise IOError(f'Process pid was non-numeric: {pid}')

  try:
    return len(os.listdir('/proc/%i/fd' % pid))
  except Exception as exc:
    raise IOError(f'Unable to check number of file descriptors used: {exc}')
...
def _inodes_for_sockets(pid):
  ...
  try:
    ...
    raise IOError(f'Unable to read our file descriptors: {exc}')
