...
class CacheController(object):
    ...
    def __init__(self, cache=None, cache_etags=True, serializer=None,
                 status_codes=None, maxsize=100000):
    ...
    def parse_cache_control(self, headers):
        ...
        cc_header = 'Cache-Control' if 'Cache-Control' in headers else 'cache-control'
        if cc_header in headers:
            ...
            parts_wo_args = [
                (name.strip().lower(), 1) for name in parts if name.find("=") == -1
            ]
    ...
    def cached_request(self, request):
        ...
        return False.
        """
        cache_url = self.cache_url(request.url, request)
        ...
        # If there isn't a max-age, check for an expires header
            ...
            if expires is not None:
                ...
                logger.debug("Freshness lifetime from expires: %i, current age: %s",
    ...
    def cache_response(self, request, response, body=None,
        ...
        if body and len(body) > self.maxsize:
            logger.debug('Discarding too big response: %s < %s  bytes', self.maxsize, len(body))
            return
